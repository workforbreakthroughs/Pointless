...
const WordDisplay: React.FC<WordDisplayProps> = ({ word, guessedLetters, revealAll = false }) => {
  ...
  const getResponsiveSizeClasses = () => {
    if (wordLength <= 6) {
      return {
        box: "w-10 h-14 sm:w-16 sm:h-22 md:w-20 md:h-28 lg:w-24 lg:h-32 xl:w-28 xl:h-36",
        text: "text-3xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl"
      };
    } else if (wordLength <= 9) {
      return {
        box: "w-8 h-12 sm:w-13 sm:h-18 md:w-16 md:h-22 lg:w-18 lg:h-26 xl:w-22 xl:h-30",
        text: "text-2xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl"
      };
    } else {
      return {
        box: "w-7 h-10 sm:w-10 sm:h-14 md:w-12 md:h-18 lg:w-14 lg:h-20 xl:w-18 xl:h-24",
        text: "text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl"
      };
    }
  };
        ...
        return (
          <div
            ...
            className={`
              ...
              ${sizes.text} font-heading shrink-0
